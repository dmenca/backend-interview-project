...
|--|--|--|--|--|
|B + 树索引|	B + 树（有序），多路平衡树结构|	绝大多数场景（等值 / 范围查询）	支持等值 / 范围 / 排序查询|	写入有额外开销|
|哈希索引|	哈希表|	等值查询（如 Memcached 类场景）|通过hash函数算出数据位置，等值查询极快(O(1))|	不支持范围 / 排序查询，仅 Memory 引擎默认|
|全文索引|	倒排索引|	文本模糊查询（如文章内容搜索）|	支持自然语言模糊查询|	仅 MyISAM/InnoDB（5.6+）支持，性能一般|
|空间索引|	R 树|	空间数据查询（如地理位置）	|支持空间范围查询|	仅 MyISAM 支持，使用极少|

InnoDB 引擎仅支持 B + 树索引（含主键 / 普通 / 唯一 / 联合索引），哈希索引需手动模拟。

### 按存储方式区分
1) 聚集索引： 主键索引就是聚集索引，聚集索引叶子节点存储索引键值（Key）和完整的行数据（Data），非叶子节点只存储索引键值（Key）+子节点指针，不存储实际数据。数据按主键索引顺序物理存储，一张表只能有一个聚集索引。

 
2)非聚集索引: 也叫二级索引，叶子节点只存索引字段值和主键值。查询二级索引还得拿着主键值去聚集索引里再查一遍，这个过程叫做回表。
...
![img.png](../../static/mysql/index-1.png)
...
1.主键索引存储完整索引键 + 数据（聚簇索引）/ 非主键索引存储索引建和主键值（二级索引）；
...
```
┌────────┬────────┬────────┬────────┬────────┐
│ 指针P1 │ Key 15 │ 指针P2 │ Key 30 │ 指针P3 │
└────────┴────────┴────────┴────────┴────────┘
    ↓                ↓                ↓
 <15的数据        15~30的数据       >30的数据
 ```
...
第1层（根）        [15, 30]
                  /    \    \
第2层（非叶）  [8,12] [20,25] [35,40]
                                          ← 只存 Key+指针
第3层（叶）  [1,5,8,12]->[15,20,25]->[30,35,40]
                                          ← 存完整数据，链表相连
```

**为什么非叶子节点不存数据?**
原因：让每层能放更多 Key，树更矮。数据库页大小固定（InnoDB 默认 16KB），非叶子节点不存数据：
```
假设：
- 每个 Key 占 8 字节（bigint）
- 每个指针占 6 字节
- 每页 16KB = 16384 字节

每页可存指针数 = 16384 / (8+6) ≈ 1170 个子节点指针
